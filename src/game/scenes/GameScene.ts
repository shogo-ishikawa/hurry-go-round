...
export class GameScene extends Phaser.Scene {
  ...
  configureInventoryE2E(input:{cargo?:Partial<GameState["cargo"]["amounts"]>;barn?:Partial<GameState["barn"]>;market?:Partial<GameState["market"]>;marketCapacity?:Partial<GameState["marketCapacity"]>;tillCoins?:number;walletCoins?:number}):void{const cargo={...this.state.cargo,amounts:{...this.state.cargo.amounts,...input.cargo}};this.state={...this.state,cargo,barn:{...this.state.barn,...input.barn},market:{...this.state.market,...input.market},marketCapacity:{...this.state.marketCapacity,...input.marketCapacity},economy:{...this.state.economy,tillCoins:input.tillCoins??this.state.economy.tillCoins,walletCoins:input.walletCoins??this.state.economy.walletCoins}};this.farmer.setCargo(cargo.amounts,cargo.capacity);this.emitState();}
