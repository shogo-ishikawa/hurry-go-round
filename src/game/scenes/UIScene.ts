...
export class UIScene extends Phaser.Scene {
  ...
  openProcessingE2E(page=0):void{this.processingPage=Math.max(0,Math.min(4,page));this.processingContentPage=0;this.openProcessing();}
  private openProcessing=(result=""):void=>{
    if(this.overlay.length||!this.lastState)return;this.overlayBase("加工場　生産管理");const state=this.lastState,w=this.scale.width,h=this.scale.height,compact=w<700||h<650;
    const tabs=["概要","建設","製粉機","ベーカリー","スタッフ"] as const,columns=compact?3:5,tabWidth=(this.panelRect.width-20)/columns,tabY=this.panelRect.y+(compact?70:82);
    ...
    const top=tabY+(Math.ceil(tabs.length/columns)-1)*52+34,bottom=this.panelRect.y+this.panelRect.height-68;this.contentRect={x:this.panelRect.x+12,y:top,width:this.panelRect.width-24,height:Math.max(60,bottom-top)};
    const tab=tabs[this.processingPage],cards:string[]=[];
    if(tab==="概要")cards.push("加工場では、農産物を価値の高い商品へ加工できます。","1. 搬入口へ原料を入れる\n2. 製粉機またはベーカリーで加工する\n3. 受取口から完成品を回収する","加工場用地\n　↓\n製粉機\n　↓\nベーカリー","麦 2 → 小麦粉 1\nとうもろこし 2 → コーンミール 1\n小麦粉 1 + たまご 1 → パン 1\n小麦粉 1 + コーンミール 1 + たまご 1 → コーンブレッド 1");
    else if(tab==="建設")for(const [id,name,guide] of [["processing-yard","加工場用地","南東の加工場入口"],["grain-mill","製粉機","加工場北側の製粉機パッド"],["bakery","ベーカリー","加工場南側のベーカリーパッド"]] as const){const a=getProcessingConstructionAvailability(id,state.processing.land,state.economy.walletCoins,state.landExpansion.eastCornFieldUnlocked,state.landExpansion.southChickenCoopUnlocked);cards.push(`【${name}】 ${a.built?"建設済み":a.available?"建設可能":"ロック中"}\n前提：${a.missingPrerequisites.length?a.missingPrerequisites.join("・"):"達成"}　費用：${a.cost}コイン\n場所：${guide}\n次に行うこと：${a.built?"設備を確認":a.reason??"建設場所へ向かう"}`);}
    else if(tab==="スタッフ")for(const [name,role,worker,built] of [["製粉スタッフ","millOperator",state.processing.millOperator,state.processing.land.millBuilt],["製パンスタッフ","baker",state.processing.baker,state.processing.land.bakeryBuilt]] as const){const cost=!worker.hired?(role==="millOperator"?450:700):worker.level===1?(role==="millOperator"?420:650):worker.level===2?(role==="millOperator"?900:1300):null;cards.push(`【${name}】 ${worker.hired?"雇用済み":"未雇用"}　Lv${worker.level}${worker.level===3?"（最大レベル）":""}\n容量 ${getProcessingWorkerCapacity(role,worker.level)}　現在の作業：${worker.publicStatus}\n${cost===null?"最大レベル":`${worker.hired?"研修":"雇用"}費用：${cost}コイン`}`);this.addModalButton(this.panelRect.x+this.panelRect.width-80,this.contentRect.y+35+(role==="baker"?70:0),120,worker.hired?worker.level===3?"最大":"研修":"雇用",()=>{this.game.events.emit(GAME_EVENTS.processingAction,role,worker.hired?"train":"hire");this.closeOverlay();this.time.delayedCall(0,()=>this.openProcessing());},built&&worker.level<3);}
    else {const key=tab==="製粉機"?"mill":"bakery",machineId=tab==="製粉機"?"grain-mill":"bakery",vm=createMachineViewModel(machineId,state.processing[key]);cards.push(`【運転状態】 ${vm.mode} / ${vm.selectedRecipe}`,`【必要原料】 ${vm.recipes.map(card=>card.formula).join("　")}`,`【入力バッファ】 ${vm.input.join(" / ")||"空"}`,`【加工中に予約済み】 ${vm.reserved.join(" / ")||"なし"}`,`【進捗】 ${vm.progress}%　残り ${vm.remaining}`,`【完成品バッファ】 ${vm.output.join(" / ")||"空"}`,`【次に行うこと】 ${vm.primaryAction}`,...vm.recipes.map(card=>`【レシピ：${card.name}】\n${card.formula}　出力 1個　${card.duration}\n状態：${card.status}`));}
    const perPage=h<430?1:h<650?2:4,maxPage=Math.max(0,Math.ceil(cards.length/perPage)-1);this.processingContentPage=Math.min(this.processingContentPage,maxPage);const shown=cards.slice(this.processingContentPage*perPage,(this.processingContentPage+1)*perPage);
    let y=this.contentRect.y+4;for(const card of shown){const text=this.add.text(this.contentRect.x+8,y,card,{fontFamily:"system-ui",fontSize:compact?"12px":"16px",color:"#49382e",backgroundColor:"#eadbb9",padding:{x:8,y:6},wordWrap:{width:this.contentRect.width-32},fixedWidth:this.contentRect.width-16}).setOrigin(0,0);this.overlay.push(text);y+=text.height+8;}
    if(result){const notice=this.add.text(this.contentRect.x+8,Math.min(y,bottom-24),result,{fontFamily:"system-ui",fontSize:"13px",color:"#7a3d24"});this.overlay.push(notice);}
    if(tab==="製粉機"||tab==="ベーカリー"){const machineId=tab==="製粉機"?"grain-mill":"bakery",machine=tab==="製粉機"?state.processing.mill:state.processing.bakery,modes=machineId==="grain-mill"?[["自動","auto"],["小麦粉","mill-flour"],["コーンミール","mill-cornmeal"],["停止","stop"]] as const:[["自動","auto"],["パン","bakery-bread"],["コーンパン","bakery-cornbread"],["停止","stop"]] as const,bw=(this.panelRect.width-20)/4;modes.forEach(([label,mode],index)=>this.addModalButton(this.panelRect.x+10+bw/2+index*bw,bottom-24,bw-4,label,()=>{this.game.events.emit(GAME_EVENTS.processingAction,machineId,mode);this.closeOverlay();this.time.delayedCall(0,()=>this.openProcessing());},machine.built));this.addModalButton(this.panelRect.x+150,bottom-70,260,"倉庫から1回分を補充",()=>{this.game.events.emit(GAME_EVENTS.processingAction,machineId,"refill");this.closeOverlay();this.time.delayedCall(0,()=>this.openProcessing());},machine.built);this.addModalButton(this.panelRect.x+this.panelRect.width-150,bottom-70,260,"完成品を倉庫へ移す",()=>{this.game.events.emit(GAME_EVENTS.processingAction,machineId,"collect");this.closeOverlay();this.time.delayedCall(0,()=>this.openProcessing());},machine.built);}
    this.addModalButton(this.panelRect.x+70,bottom+30,112,"前へ",()=>{this.closeOverlay();this.processingContentPage=Math.max(0,this.processingContentPage-1);this.openProcessing();},this.processingContentPage>0);this.addModalButton(this.panelRect.x+this.panelRect.width-70,bottom+30,112,"次へ",()=>{this.closeOverlay();this.processingContentPage=Math.min(maxPage,this.processingContentPage+1);this.openProcessing();},this.processingContentPage<maxPage);
    const indicator=this.add.text(w/2,bottom+20,`${this.processingContentPage+1} / ${maxPage+1}ページ`,{fontFamily:"system-ui",fontSize:"13px",color:"#49382e"}).setOrigin(.5);this.overlay.push(indicator);this.addModalButton(this.panelRect.x+this.panelRect.width-64,this.panelRect.y+30,104,"閉じる",()=>this.closeOverlay());
  };
  ...
  getProcessingPanelE2ESummary(){const machine=this.lastState?(this.processingPage===2?createMachineViewModel("grain-mill",this.lastState.processing.mill):this.processingPage===3?createMachineViewModel("bakery",this.lastState.processing.bakery):null):null;return{page:this.processingPage,pageName:["概要","建設","製粉機","ベーカリー","スタッフ"][this.processingPage],scrollPosition:this.processingContentPage,panel:{...this.panelRect},content:{...this.contentRect},visibleText:this.getVisibleTextDiagnostics(),buttons:this.modalButtons.map(button=>button.getSerializableRect()),machine};}
