...
import { createProcessingPlanViewModel, DEFAULT_PROCESSING_PLANS, setRecipeTargetCycles, type RecipePlan } from "../logic/processingPlan";
...
目標 ${c.targetCycles}回　不足 ${format(c.missingInputs)}`));}
    ...
    if(tab==="製粉機"||tab==="ベーカリー"){const machineId:MachineId=tab==="製粉機"?"grain-mill":"bakery",recipeIds=Object.values(RECIPES).filter(r=>r.machine===machineId).map(r=>r.id);recipeIds.forEach((recipeId,index)=>{const current=this.processingPlans[machineId].targetCyclesByRecipe[recipeId]??0,sliderWidth=Math.max(80,Math.min(220,(this.panelRect.width-220)/2)),sy=bottom-72-index*54;this.addModalButton(this.panelRect.x+40,sy,48,"−",()=>this.changePlan(machineId,recipeId,current-1),current>0);const slider=new DiscreteSlider(this,this.panelRect.x+this.panelRect.width/2,sy,sliderWidth,current,0,10,value=>this.changePlan(machineId,recipeId,value));this.planSliders.push(slider);this.overlay.push(slider);this.addModalButton(this.panelRect.x+this.panelRect.width-40,sy,48,"＋",()=>this.changePlan(machineId,recipeId,current+1),current<10);});}
