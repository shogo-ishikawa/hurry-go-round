...
export class UIScene extends Phaser.Scene {
  ...
  private openProcessing=(result=""):void=>{
    ...
    else if(tab==="スタッフ")for(const [name,worker] of [["製粉スタッフ",state.processing.millOperator],["製パンスタッフ",state.processing.baker]] as const)cards.push(`【${name}】 ${worker.hired?"雇用済み":"未雇用"}　Lv${worker.level}${worker.level===3?"（最大レベル）":""}\n容量 ${worker.level?4+worker.level*2:0}　現在の作業：${worker.publicStatus}\n研修費用：${worker.level===1?260:worker.level===2?620:worker.level===3?"最大":"雇用後に表示"}`);
