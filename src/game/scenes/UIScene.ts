...
export class UIScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.versionText = this.add.text(0, 0, "v0.9.9", {
      ...style,
      fontSize: "14px",
      color: "#755c49",
    });
  ...
  private openProcessing=(pageOrResult:number|string=""):void=>{
    if(typeof pageOrResult==="number"){this.processingPage=pageOrResult;pageOrResult="";}if(this.overlay.length||!this.lastState)return;this.processingPlans={"grain-mill":{targetCyclesByRecipe:{...this.lastState.processing.mill.recipeTargetCycles}},bakery:{targetCyclesByRecipe:{...this.lastState.processing.bakery.recipeTargetCycles}}};this.planSliders=[];this.overlayBase("加工場　生産管理");const state=this.lastState,w=this.scale.width,h=this.scale.height,compact=w<700||h<650;
...
このレシピを設定`);}}
    else if(tab==="完成品"){for(const [machineId,machine] of [["grain-mill",state.processing.mill],["bakery",state.processing.bakery]] as const){const recipes=Object.values(RECIPES).filter(r=>r.machine===machineId);cards.push(`${machineId==="grain-mill"?"製粉機":"ベーカリー"}　完成品置き場 ${Object.values(machine.output.amounts).reduce((a,b)=>a+b,0)}/${machine.output.capacity}\n${format(machine.output.amounts)}\n\n今回の仕込み\n${recipes.map(r=>`${r.publicName} ${machine.currentPlanCompletedCycles[r.id]??0} / 計画${machine.recipeTargetCycles[r.id]??0}`).join("\n")}\n\n累計\n${recipes.map(r=>`${r.publicName} ${machine.completedByRecipe[r.id]??0}`).join("\n")}\n旧バージョンでの製造 ${machine.legacyUnattributedCycles}回\n最後に完成 ${machine.lastCompletion?RECIPES[machine.lastCompletion.recipeId].publicName:"なし"}\n直近の履歴\n${machine.recentHistory.slice(0,5).map(item=>`${RECIPES[item.recipeId].publicName} 1個`).join("\n")||"なし"}`);}}
...
目標 ${c.targetCycles}回　不足 ${format(c.missingInputs)}`));}
    ...
    if(tab==="完成品"){this.addModalButton(this.panelRect.x+this.panelRect.width/2-100,bottom-24,190,"製粉品を倉庫へ",()=>this.game.events.emit(GAME_EVENTS.processingAction,"grain-mill","collect"));this.addModalButton(this.panelRect.x+this.panelRect.width/2+100,bottom-24,190,"製パン品を倉庫へ",()=>this.game.events.emit(GAME_EVENTS.processingAction,"bakery","collect"));}
