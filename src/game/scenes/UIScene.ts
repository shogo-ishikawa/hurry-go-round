...
export class UIScene extends Phaser.Scene {
  ...
  private handleModalKey=(event:KeyboardEvent):void=>{if(!this.collectionOpenState)return;if(event.key==="Escape"){event.preventDefault();this.closeOverlay();return;}if(event.key==="Tab"){event.preventDefault();if(!this.modalButtons.length)return;this.modalFocus=(this.modalFocus+(event.shiftKey?-1:1)+this.modalButtons.length)%this.modalButtons.length;this.modalButtons.forEach((b,i)=>b.setFocused(i===this.modalFocus));}else if((event.key==="Enter"||event.key===" ")&&this.modalFocus>=0){event.preventDefault();this.modalButtons[this.modalFocus].trigger();}};
