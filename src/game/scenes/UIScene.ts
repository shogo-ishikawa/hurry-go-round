...
export class UIScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.versionText = this.add.text(0, 0, "v0.9.4", {
      ...style,
      fontSize: "14px",
      color: "#755c49",
    });
    ...
    this.game.events.on(GAME_EVENTS.processingRange,(visible:boolean)=>this.processingButton.setVisible(visible&&!this.overlay.length));
    this.game.events.on(GAME_EVENTS.processingOpen,this.openProcessing,this);
    this.game.events.on(GAME_EVENTS.collectionRange,(visible:boolean)=>this.collectionButton.setVisible(visible&&!this.overlay.length));
  ...
  private openCollection=():void=>{if(this.overlay.length||!this.lastState)return;this.overlayBase("集配所");const state=this.lastState,n=state.collectionNetwork,w=this.scale.width,h=this.scale.height,c=n.courier,stage:Record<typeof c.stage,string>={"not-hired":"未雇用","idle-at-hub":"集配所で待機","select-source":"集荷先を確認中","moving-to-source":"集配ボックスへ移動中","loading":"積み込み中","waiting-for-batch":"まとめ積み待機中","select-destination":"配送先を確認中","moving-to-processing":"加工場へ配送中","moving-to-barn":"倉庫へ配送中","unloading-processing":"加工場で荷下ろし中","unloading-barn":"倉庫で荷下ろし中","returning-to-hub":"集配所へ帰還中"};const load=Object.values(c.carried).reduce((sum,value)=>sum+value,0),intake=Object.values(n.processingIntake.amounts).reduce((sum,value)=>sum+value,0);const info=this.add.text(w/2,h/2-230,`配送モード：${n.routingMode==="auto"?"自動":n.routingMode==="processing-first"?"加工場優先":"倉庫優先"}\n集配スタッフ：${c.hired?`Lv${c.level}　${stage[c.stage]}`:"未雇用"}\n積載：${load} / ${c.capacity}\n\n麦畑集配箱　${n.boxes.wheat.amounts.wheat} / 24\n東農地集配箱　${n.boxes.corn.amounts.corn} / 28\n鶏小屋集配箱　${n.boxes.egg.amounts.egg} / 18\n加工場受入箱　${intake} / 36`,{fontFamily:"system-ui",fontSize:w<600?"15px":"19px",color:"#49382e",lineSpacing:8}).setOrigin(.5,0);this.overlay.push(info);this.button(w/2-180,h/2+130,c.hired?c.level===3?"最大レベル":"研修":"雇用 480",()=>{if(!c.hired||c.level<3)this.game.events.emit(GAME_EVENTS.collectionAction,c.hired?"train":"hire");this.closeOverlay();});this.button(w/2,h/2+130,"配送モード切替",()=>{const next=n.routingMode==="auto"?"processing-first":n.routingMode==="processing-first"?"barn-first":"auto";this.game.events.emit(GAME_EVENTS.collectionAction,"mode",next);this.closeOverlay();});this.button(w/2+180,h/2+130,"麦を緊急移送",()=>{this.game.events.emit(GAME_EVENTS.collectionAction,"flush","wheat");this.closeOverlay();});this.button(w-75,50,"閉じる",()=>this.closeOverlay());};
  private openProcessing=():void=>{if(this.overlay.length||!this.lastState)return;this.overlayBase("加工場");const state=this.lastState,w=this.scale.width,h=this.scale.height;const status=(key:"mill"|"bakery")=>{const m=state.processing[key];return !m.built?"未建設":m.activeCycle?`${m.activeCycle.recipeId}　残り ${(m.activeCycle.remainingMs/1000).toFixed(1)}秒`:m.enabled?"原料待ち":"停止中";};const info=this.add.text(w/2,h/2-190,`コイン ${state.economy.walletCoins}\n\n製粉機 Lv${state.processing.mill.level}　${status("mill")}\n入力　麦 ${state.processing.mill.input.amounts.wheat}/${state.processing.mill.input.capacity}　とうもろこし ${state.processing.mill.input.amounts.corn}/${state.processing.mill.input.capacity}\n出力　小麦粉 ${state.processing.mill.output.amounts.flour}　コーンミール ${state.processing.mill.output.amounts.cornmeal}\n\nベーカリー Lv${state.processing.bakery.level}　${status("bakery")}\n入力　小麦粉 ${state.processing.bakery.input.amounts.flour}　コーンミール ${state.processing.bakery.input.amounts.cornmeal}　たまご ${state.processing.bakery.input.amounts.egg}\n出力　パン ${state.processing.bakery.output.amounts.bread}　コーンブレッド ${state.processing.bakery.output.amounts.cornbread}`,{fontFamily:"system-ui",fontSize:w<600?"14px":"18px",color:"#49382e",align:"left",lineSpacing:7}).setOrigin(.5,0);this.overlay.push(info);if(state.processing.mill.built){this.button(w/2-170,h/2+170,"製粉：自動",()=>{this.game.events.emit(GAME_EVENTS.processingAction,"grain-mill","auto");this.closeOverlay();});this.button(w/2,h/2+170,"小麦粉優先",()=>{this.game.events.emit(GAME_EVENTS.processingAction,"grain-mill","mill-flour");this.closeOverlay();});this.button(w/2+170,h/2+170,"製粉：停止",()=>{this.game.events.emit(GAME_EVENTS.processingAction,"grain-mill","stop");this.closeOverlay();});}if(state.processing.bakery.built){this.button(w/2-120,h/2+225,"製パン：自動",()=>{this.game.events.emit(GAME_EVENTS.processingAction,"bakery","auto");this.closeOverlay();});this.button(w/2+120,h/2+225,"製パン：停止",()=>{this.game.events.emit(GAME_EVENTS.processingAction,"bakery","stop");this.closeOverlay();});}this.button(w-75,50,"閉じる",()=>this.closeOverlay());};
  ...
  private cleanup(): void {
    ...
    this.game.events.off(GAME_EVENTS.contractRange, this.showContractButton, this);this.game.events.off(GAME_EVENTS.contractOpen,this.openContracts,this);this.game.events.off(GAME_EVENTS.operationsOpen,this.openOperations,this);this.game.events.off(GAME_EVENTS.processingOpen,this.openProcessing,this);this.game.events.off(GAME_EVENTS.collectionOpen,this.openCollection,this);this.game.events.off("save-status",this.updateSaveStatus,this);this.input.keyboard?.off("keydown-ESC",this.togglePause,this);this.input.keyboard?.off("keydown-P",this.togglePause,this);
