...
export function createPersistedSnapshot(state: GameState, runtime: RuntimeSnapshot): PersistedGameSnapshot {
  return {
    ...
    landExpansion: { ...state.landExpansion, wheatFieldLevel: state.landExpansion.wheatFieldLevel ?? 0 }, livestock: { ...state.livestock, coopLevel:state.coopLevel, eggRemainingMs: runtime.eggRemainingMs ?? 4500 }, upgrades: { ...state.upgrades },
