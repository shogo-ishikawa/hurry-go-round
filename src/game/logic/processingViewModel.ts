...
export interface RecipeCardViewModel { id:RecipeId; name:string; formula:string; outputQuantity:number; duration:string; status:string; missingIngredients:string[]; modePreventsSelection:boolean }
...
export function createMachineViewModel(id:MachineId,machine:ProcessingMachineState):MachineViewModel{
 ...
 return{id,name:id==="grain-mill"?"製粉機":"ベーカリー",mode:!machine.enabled?"停止":machine.selectedMode==="auto"?"自動":RECIPES[machine.selectedMode].publicName,selectedRecipe:active?RECIPES[active.recipeId].publicName:machine.selectedMode==="auto"?"自動選択":RECIPES[machine.selectedMode].publicName,input:amountLines(machine.input.amounts),reserved:active?amountLines(active.reservedInputs):[],output:amountLines(machine.output.amounts),progress:active?Math.round((1-active.remainingMs/active.durationMs)*100):0,remaining:active?`${(active.remainingMs/1000).toFixed(1)}秒`:"—",primaryAction:getMachineNextAction(machine),recipes:recipeIds.map(recipeId=>{const recipe=RECIPES[recipeId],missingIngredients=RESOURCE_IDS.filter(resource=>machine.input.amounts[resource]<recipe.inputs[resource]).map(getResourceName);return{id:recipeId,name:recipe.publicName,formula:formula(recipe),outputQuantity:RESOURCE_IDS.reduce((n,resource)=>n+recipe.outputs[resource],0),duration:`${(recipe.baseDurationMs/1000).toFixed(1)}秒`,status:recipeStatus(machine,recipe),missingIngredients,modePreventsSelection:machine.selectedMode!=="auto"&&machine.selectedMode!==recipeId};})};
}
export function createProcessingOverview(){return{title:"概要",explanation:"加工場では、農産物を価値の高い商品へ加工できます。",steps:["搬入口へ原料を入れる","製粉機またはベーカリーで加工する","受取口から完成品を回収する"],recipes:(Object.keys(RECIPES) as RecipeId[]).map(id=>({id,name:RECIPES[id].publicName,formula:formula(RECIPES[id])}))};}
