...
export class ProcessingFacilityView {
  ...
  private readonly stations:{yard:InteractionStationView;millBuild:InteractionStationView;bakeryBuild:InteractionStationView;millInput:InteractionStationView;millOutput:InteractionStationView;bakeryInput:InteractionStationView;bakeryOutput:InteractionStationView;management:InteractionStationView;millPlanMenu:InteractionStationView;bakeryPlanMenu:InteractionStationView};
  ...
  update(state:GameState,delta:number):void{const {land,mill,bakery}=state.processing,east=state.landExpansion.eastCornFieldUnlocked,coop=state.landExpansion.southChickenCoopUnlocked;
    ...
    this.millHouse.setVisible(mill.built);this.gear.setVisible(mill.built);this.bakeryHouse.setVisible(bakery.built);this.stations.management.setVisible(land.yardUnlocked);this.stations.millPlanMenu.setVisible(mill.built).setEnabled(mill.built);this.stations.bakeryPlanMenu.setVisible(bakery.built).setEnabled(bakery.built);
    ...
    this.millStorage.setVisible(mill.built).setText(`計画：小麦粉 6回 / コーンミール 6回　目標：麦12 とうもろこし12
現在：麦${mill.input.amounts.wheat} とうもろこし${mill.input.amounts.corn}　不足：麦${Math.max(0,12-mill.input.amounts.wheat)} とうもろこし${Math.max(0,12-mill.input.amounts.corn)}
原料　麦 ${mill.input.amounts.wheat}　とうもろこし ${mill.input.amounts.corn}　${getBufferTotal(mill.input)}/${mill.input.capacity}\n${getBufferTotal(mill.output)?`完成品　小麦粉 ${mill.output.amounts.flour}　ミール ${mill.output.amounts.cornmeal}`:"完成品はまだありません"}`);
    this.bakeryStorage.setVisible(bakery.built).setText(`計画：パン 3回 / コーンブレッド 3回　目標：粉6 ミール3 卵6
現在：粉${bakery.input.amounts.flour} ミール${bakery.input.amounts.cornmeal} 卵${bakery.input.amounts.egg}　不足：粉${Math.max(0,6-bakery.input.amounts.flour)} ミール${Math.max(0,3-bakery.input.amounts.cornmeal)} 卵${Math.max(0,6-bakery.input.amounts.egg)}
原料　粉 ${bakery.input.amounts.flour}　ミール ${bakery.input.amounts.cornmeal}　卵 ${bakery.input.amounts.egg}　${getBufferTotal(bakery.input)}/${bakery.input.capacity}\n${getBufferTotal(bakery.output)?`完成品　パン ${bakery.output.amounts.bread}　コーンパン ${bakery.output.amounts.cornbread}`:"完成品はまだありません"}`);
