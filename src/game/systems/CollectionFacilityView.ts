...
export class CollectionFacilityView{
 ...
 showTransfer(source:CollectionSourceId,direction:"deposit"|"withdraw",amount:number):void{const p=COLLECTION_FACILITIES[source].guidePoint,symbol=`${direction==="deposit"?"→":"←"} +${amount}`,text=this.scene.add.text(p.x+(direction==="deposit"?-78:78),p.y,symbol,{fontFamily:"system-ui",fontSize:"18px",fontStyle:"bold",color:direction==="deposit"?"#297c78":"#b9573f",backgroundColor:"#fff4d8"}).setOrigin(.5).setDepth(2200);const reduced=globalThis.matchMedia?.("(prefers-reduced-motion: reduce)").matches??false;this.scene.tweens.add({targets:text,y:reduced?p.y:p.y-35,alpha:0,duration:reduced?120:650,onComplete:()=>text.destroy()});}
