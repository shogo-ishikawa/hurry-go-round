...
function migrateResources(payload:Record<string,unknown>):PersistedGameSnapshot{
  ...
  p.landExpansion.cornFieldLevel??=0; p.collectionNetwork=createCollectionNetwork(); if(object(p.operations)){const id=p.operations.lastSelectedFacilityId;p.operations.lastSelectedFacilityId=id==="operations-office"?"training-lodge":id==="training-lodge"?id:null;} return p as unknown as PersistedGameSnapshot;
}
export async function migrateSaveEnvelope(input:unknown):Promise<ValidationResult<SaveEnvelope>>{
 if(!object(input)||typeof input.schemaVersion!=="number")return{ok:false,errors:["schema version missing"]};if(input.schemaVersion>SAVE_SCHEMA_VERSION)return{ok:false,errors:["このセーブデータは新しいバージョンで作成されています"]};if(input.schemaVersion===SAVE_SCHEMA_VERSION)return validateEnvelope(input);if(![1,2,3].includes(input.schemaVersion)||!object(input.payload)||typeof input.checksum!=="string")return{ok:false,errors:["unsupported schema version"]};if(!(await verifyChecksum(input.payload,input.checksum)))return{ok:false,errors:["checksum mismatch"]};
 ...
 const payload=migrateResources(old);const envelope={...input,schemaVersion:SAVE_SCHEMA_VERSION,gameVersion:GAME_VERSION,payload,checksum:await checksumPayload(payload)} as SaveEnvelope;const validated=await validateEnvelope(envelope);return validated.ok?{...validated,warnings:[...warnings,"schema 4 の集配ネットワークを追加しました"]}:validated;
}
